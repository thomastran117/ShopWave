...
public interface ReturnRepository extends JpaRepository<Return, Long> {
    ...
    List<Return> findAllByOrderId(long orderId);
    ...
    @Query("SELECT r FROM Return r WHERE r.order.id = :orderId AND r.status NOT IN ('REJECTED','FAILED')")
    ...
    @Query("SELECT DISTINCT r FROM Return r JOIN r.items ri JOIN ri.orderItem oi " +
           "WHERE oi.product.company.id = :companyId")
